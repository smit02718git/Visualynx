...
export default async function Page({ searchParams }: PageProps) {
  ...
  if (concept) {
    const validation = await validateTopic('chemistry', concept)
      ...
      return (
        <main className="flex min-h-screen items-center justify-center bg-slate-50 p-6 text-center">
          <div className="max-w-lg rounded-2xl border border-[#e1e7ef] bg-white p-8 shadow-sm">
            <p className="text-lg font-semibold text-[#1d2433]">{validation.message || 'This concept is not related to chemistry.'}</p>
            <Link href="/workspace/chemistry" className="mt-6 inline-flex rounded-xl bg-[#d8842d] px-4 py-2 text-sm font-semibold text-white">Choose another concept</Link>
          </div>
        </main>
      )
    }
    const { visualization, explaination, formulas, mistakes } = await getWorkspaceData('chemistry', concept)
    ...
    return (
      <main className="zoom-[0.8]">
        <header className="border-b border-[#dfe4ec] bg-[#f3f4f6]/95 backdrop-blur-sm">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-2">
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              <p className="text-sm text-[#5d6982]">chemistry</p>
            </div>

            <div className="flex gap-4 items-center">
              <Link href="/workspace/chemistry">
        ...
        <main className="flex min-h-screen justify-left bg-slate-50 p-6 gap-7">
          ...
          <WorkspaceSectionNav subject="chemistry" concept={concept} learnContent={<ConceptCard data={explaination} />} formulasContent={<FormulaSection data={formulas} />} mistakesContent={<CommonMistakesSection data={mistakes} />} chatContent={<ChatSection subject="chemistry" />}/>
        </main>
      </main>
    );
  }
