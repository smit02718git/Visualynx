...
export default async function Page({ searchParams }: PageProps) {
    ...
    return (
      <main className="zoom-[0.8]">
        <header className="border-b border-[#dfe4ec] bg-[#f3f4f6]/95 backdrop-blur-sm">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-2">
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              <p className="text-sm text-[#5d6982]">maths</p>
            </div>

            <div className="flex gap-4 items-center">
              <Link href="/workspace/maths">
        ...
        <main className="flex min-h-screen justify-left bg-slate-50 p-6 gap-7">
          ...
          <WorkspaceSectionNav learnContent={<ConceptCard data={explaination} />} formulasContent={<FormulaSection data={formulas} />} mistakesContent={<CommonMistakesSection data={mistakes} />} chatContent={<ChatSection subject="maths" />}/>
        </main>
      </main>
    );
  }
  ...
  const config = getSubjectPageConfig('maths')
